...
for i in range(2):
    for j in range(3):
        ...
        ax[i, j].set_ylim(-.2, 12.2)
        ax[i, j].set_xlim(-.2, 12.2)
        ax[i, j].set_xticks([0, 2, 4, 6, 8, 10, 12])
        ax[i, j].set_yticks([0, 2, 4, 6, 8, 10, 12])
...
fig.legend(handles=[blue, orange, green], bbox_to_anchor=(.9, .6), loc="center", borderaxespad=0., frameon=False,
           fontsize=14)
